...
namespace SharpHoning
{
    ...
    public class Program
    {
        ...
        List<result> renormalizeSimple(List<result> reslist, ref double maxs, ref double mins, ref double minef, ref double maxef)
        {
            ...
            //Renormalize response
            double maxSurprise = 0.0;
            double minSurprise = double.MaxValue;
            double maxEfficiency = 0.0;
            double minEfficiency = double.MaxValue;
            foreach (result r in reslist)
            {
                if (r.surprise > maxSurprise)
                    maxSurprise = r.surprise;
                if (r.surprise < minSurprise)
                    minSurprise = r.surprise;
                if (r.efficiency > maxEfficiency)
                    maxEfficiency = r.efficiency;
                if (r.efficiency < minEfficiency)
                    minEfficiency = r.efficiency;
            }
            ...
            foreach (result r in reslist)
            {
                ...
                double normSurprise = normalize(r.surprise, maxSurprise, minSurprise);
                double normEfficiency = normalize(r.efficiency, maxEfficiency, minEfficiency);
                double fit = fitness(normSurprise, normEfficiency);

                nr.highEff = maxEfficiency;
                nr.minEff = minEfficiency;
                nr.highSurp = maxSurprise;
                nr.minSurp = minSurprise;
