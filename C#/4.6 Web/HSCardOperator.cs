...
ref KNNEfficiency knn)
        ...
        public List<HoningNode<String>> getWindow(List<HoningNode<String>> input, int size)
        {
            List<HoningNode<String>> ret = new List<HoningNode<string>>();
            ...
            int realWindowSize = (int)((double)input.Count * percent);

            if (realWindowSize < 1)
                realWindowSize = 2;

            if (realWindowSize >= input.Count)
                return input;

            int startPosition = rand.Next(1, input.Count - realWindowSize);

            for (int i = startPosition - 1; i < startPosition - 1 + realWindowSize; i++)
            {
                ret.Add(input[i]);
            }
 ...
 out double normEfficiency)
        {
            ...
            while (cards.Count > 0)
            {
                ...
                if (onlyBackwardInflation)
                {
                    foreach (HoningNode<String> n in network[card.name.ToLower()].parents.Values)
                    {
                        if (abilityFilter.ContainsKey(n.holder) && n.mark == "internal")
                        {
                            foreach (HoningNode<String> p in n.parents.Values)
                            {
                                if (!combo.combo.ContainsKey(p.holder) && !toCheck.ContainsKey(p.holder) && p.mark == "terminal")
                                {
                                    searchSpace.Add(p);
                                }
                            }
                    ...
                    foreach (HoningNode<String> n in network[card.name.ToLower()].clique.Values)
                    {
                        if (n.mark == "essential")
                        {
                            foreach (HoningNode<String> p in n.parents.Values)
                            {
                                if (p.mark == "internal" && abilityFilter.ContainsKey(p.holder))
                                {
                                    foreach (HoningNode<String> pp in p.parents.Values)
                                    {
                                        if (pp.mark == "terminal")
                                        {
                                            CardObject obj = cardTable[pp.holder];
                                            CardAbility ab = ExtractAbilityObject(ref obj, p.holder);

                                            if (ab.target[0] == n.holder)
                                                if (!combo.combo.ContainsKey(pp.holder) && !toCheck.ContainsKey(pp.holder) && pp.mark == "terminal")
                                                {
                                                    searchSpace.Add(pp);
                                                }
                                        }
                ...
                if (searchSpace.Count > 0)
                {
                    ...
                    searchSpace = getWindow(searchSpace, windowSize);
                    ...
                    foreach (HoningNode<String> parent in searchSpace)
                    {
                        ...
                        //Console.WriteLine("KNN Execution time: " + sw.ElapsedMilliseconds);
                        //sw.Stop();
        ...
        public Dictionary<String, HoningNode<String>> GenerateCardClusterNewRandom(
         String seed,
         ref Dictionary<String, CardObject> cardTable,
         ref HoningNetwork<String> net,
         ref Dictionary<String, String> abilityFilter,
         ...
         out double normEfficiency)
        {
            ...
            while (cards.Count > 0)
            {
                ...
                if (searchSpace.Count > 0)
                {
                    ...
                    higherNode = searchSpace.ElementAt(randomParent);
                    higherExpansion = higherNode.holder;

                    if (combo.combo.ContainsKey(higherNode.holder))
                        continue;
